...
colnames(df) <- c("date","Chl_export","H2O_export")
...
Qcorrected = raw_data$Q - Qmin
Qmax = max(raw_data$Q) ; Qmin = min(raw_data$Q)


peak_time = (which(raw_data$Q == Qmax))*15

results <- list(date_storm,export_Chl,export_H2O)
results = as.data.frame(as.vector(results))
colnames(results) <- c("date","Chl_export","H2O_export") 
df<-rbind(df,results)
