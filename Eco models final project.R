...
###################model 1:simple daily nitrate uptake###############

options(mc.cores = parallel::detectCores())##this code allows stan to count the number of cores and run 4 chains on each core.  
#In the the case of SL regression, it is so fast you will nver see it.  In the case of a 1000 paramter model, you will really want parallel processing of chains
...
cat("
    ...
    data {
    int <lower = 1> N;
    vector[N] Q;//discharge 
    vector[N] A;//channel area
    vector[N] conc;//nitrate conc.
    }
    
    parameters {
    real intercept;                // intercept aka. predawn max peak
    real U;                // uptake
    ...
    model {
    for (i in 1:N){
    conc[i]~ normal(intercept-((A[i]*U)/Q[i]), sigma); // likelihood
    }
    U~normal(0,120); //priors
